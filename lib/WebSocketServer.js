...
WebSocketServer.prototype.close = function(callback) {
  ...
  var error = null;
  try {
    this.clients.forEach(function(item, key){
      item.terminate();
    });
  }
  catch (e) {
...
function handleHybiUpgrade(req, socket, upgradeHead, cb) {
  ...
  var completeHybiUpgrade2 = function(protocol) {
    ...
    self.emit('headers', headers, req, socket);
    ...
    if (self.options.clientTracking) {
      ...
      md5sum.update((req.headers['games-customerid'] || '_') + (req.headers['x-real-ip'] || '_'), 'utf8');
      client.id = md5sum.digest('hex');
      if(self.clients.has(client.id)){
        self.clients.get(client.id).close(1002, '');
      }
